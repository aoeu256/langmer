...
sounds = ['click1', 'click2', 'click3', 'success', 'failure']

Sound = {}
for i in sounds
	Sound[i] = new SoundEffect i
Sound.click = {play: -> Sound['click'+randItem([1, 2, 3])].play()}
...
class Game
	...
	popupStories: (it) ->
		...
		@call 'get_stories', it, (data, a, b) => 
			...
			$('#storypopup a[data-role="button"]').unbind('vclick').bind('vclick') (e) =>
	...
	goal_start: -> # Please finish
		...
		$('#finishbut').unbind('vclick').vclick =>
			@switchPage 'start'
	...
	start_start: ->
		...
		for key, tex of string2id
			$('#'+key).text(tex)
		$('#reviewbut').unbind('vclick').bind('vclick') =>
	...
	gameloop: (mode='show', round=1, error=false, wordsource, postsave=false) ->
		...
		if mode is 'match'
			$.event.spceial.swipe.durationThreshold = 2000
			$.event.special.swipe.horizontalDistanceThreshold = 30
			$.event.special.swipe.verticalDistanceThreshold = 500
			$.event.special.handleSwipe = (start, stop) ->
				return if @mode isnt 'match'
		...
		if mode is 'show' and $('#skipbut').length is 0
			$but = $('<a id="skipbut" data-role="button">Skip unselected items</a>')
			$but.appendTo('#main')
			$but.trigger 'create'
			$but.button()
	...
	drawTab: (mode='show') ->
		...
		for column in @cols
			uicls = 'ui-block-'+ col2block[column] + ' gridcol'
			uicls += ' othercol' if col2block[column] isnt 'a'
			$block = $("<div/>", {"id":"#{column}_col", "class":uicls})
			...
			$lst.append($hed)
			for i in [0..@Nitems-1]
				$li = $ '<li/>', {"data-theme":"c", "id":column+'_'+i, "data-mini":"true", "class":"ui-disabled mybutton"}
	...
	enterTextMode: ->
		...
		$('.myinput').unbind('vclick').bind('click') (e) => 
	...
	updateButtons: ->
		$("#game .item").unbind('vclick').bind('vclick') (e) =>
			if @locked
				return
...
class App
	...
	changeNavi: (e) ->		
		...
		if mode is 'Graphs'
			...
			@changePage 'listpage'
			$('#newbutton').unbind('vclick').bind('vclick') ->
